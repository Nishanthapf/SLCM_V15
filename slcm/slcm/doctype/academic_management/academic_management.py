...
def get_classes(filters=None):
    ...
    classes = frappe.get_list('Student Group',
        fields=['name', 'student_group_name', 'course', 'academic_term', 'faculty', 'class_type', 'max_strength', 'section', 'status'],
...
@frappe.whitelist()
def create_classes_by_section(department, program, academic_year, batch, academic_term, course, class_type, faculty):
...
def process_bulk_class_creation(department, program, academic_year, batch, academic_term, course, class_type, faculty, user):
    sections = frappe.get_all('Program Batch Section', 
        filters={
            'department': department,
            'program': program,
            'academic_year': academic_year,
