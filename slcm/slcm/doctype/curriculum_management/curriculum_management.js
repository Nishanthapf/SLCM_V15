...
frappe.ui.form.on('Curriculum Management', {
    refresh: function (frm) {
        ...
        frm.page.set_primary_action('Save', function () {
            if (frm.doc.setup_curriculum_tab) {
                // If in Setup tab, we try to save both
                // But let's prioritize saving the Settings first via standard save
                // Then triggering the backend curriculum save

                // Trigger standard form save
                frm.save().then(() => {
                    // Then save curriculum data
    ...
    load_curriculum: function (frm) {
        if (frm.doc.department && frm.doc.program && frm.doc.academic_year) {
            ...
            $(frm.fields_dict.ui_container.wrapper).html('<div class="text-muted text-center p-5">Please select Department, Program and Year</div>');
        }
    },
    ...
    render_ui: function (frm) {
			...
			<div class="curriculum-manager">
				<div class="clearfix mb-3">
					...
					<div class="text-muted small float-left">
						Configure courses for ${frm.curriculum_data.program} (${frm.curriculum_data.academic_year})
					</div>
				</div>
        ...
        // We assume 8 semesters for now
        const semesters = [1, 2, 3, 4, 5, 6, 7, 8];

        semesters.forEach(sem => {
            const semesterName = `Semester ${sem}`;
            const coursesInSem = (frm.curriculum_data.curriculum_courses || []).filter(c => c.semester === semesterName);

            const semHtml = `
				<div class="card">
					<div class="card-header" id="heading${sem}">
						<h2 class="mb-0">
							<button class="btn btn-link btn-block text-left collapsed" type="button" data-toggle="collapse" data-target="#collapse${sem}">
								Semester ${sem}
							</button>
						</h2>
					</div>

					<div id="collapse${sem}" class="collapse" data-parent="#accordionSemesters">
						<div class="card-body">
							<div class="enrollment-types-container-${sem}"></div>
						</div>
					</div>
				</div>
			`;
            ...
            const $etContainer = $semBlock.find(`.enrollment-types-container-${sem}`);

            enrollment_types.forEach(et => {
                const etCourses = coursesInSem.filter(c => c.enrollment_type === et.enrollment_type);
                ...
                const etHtml = `
					<div class="enrollment-section mb-4">
						<div class="d-flex justify-content-between align-items-center mb-2">
							...
							<div>
								<button class="btn btn-xs btn-default btn-add-course" data-sem="${semesterName}" data-et="${et.enrollment_type}">+ Add Course</button>
								${et.enrollment_type !== 'Core' ? `<button class="btn btn-xs btn-default btn-add-cluster ml-1" data-sem="${semesterName}" data-et="${et.enrollment_type}">+ Add Cluster</button>` : ''}
							</div>
						</div>
						<div class="course-list list-group" id="list-${sem}-${et.enrollment_type.replace(/[^a-zA-Z0-9]/g, '')}">
							${etCourses.map((course, idx) => render_course_item(course, idx, semesterName, et.enrollment_type)).join('')}
						</div>
					</div>
				`;
        ...
        $container.on('click', '.remove-course', function (e) {
            ...
            let deleted = false;
            frm.curriculum_data.curriculum_courses = frm.curriculum_data.curriculum_courses.filter(c => {
                if (deleted) return true; // already deleted one
                if (c.semester === sem && c.enrollment_type === et) {
                    if (isCluster && c.course_group_type === 'Cluster' && c.cluster_name === clusterName) {
                        deleted = true;
                        return false;
                    }
    ...
    const d = new frappe.ui.Dialog({
        ...
        primary_action: function (values) {
            ...
            frappe.db.get_value('Course', values.course, 'credit_value').then(r => {
                ...
                if (!frm.curriculum_data.curriculum_courses) frm.curriculum_data.curriculum_courses = [];
...
function add_cluster_dialog(frm, semester, enrollment_type) {
    const d = new frappe.ui.Dialog({
        ...
        fields: [
            ...
            { label: 'Min Courses', fieldname: 'min_courses', fieldtype: 'Int', reqd: 1, default: 1 },
            { label: 'Max Courses', fieldname: 'max_courses', fieldtype: 'Int', reqd: 1, default: 1 }
        ],
        primary_action_label: 'Add',
        primary_action: function (values) {
            if (!frm.curriculum_data.curriculum_courses) frm.curriculum_data.curriculum_courses = [];
