...
class IDCardGeneration(Document):
	def autoname(self):
		# Master Level: Enforce Series in Python
		series_map = {
			"Student": "STU-.#####",
			"Faculty": "FAC-.#####",
			"Driver": "DRV-.#####",
		...
		# Set naming_series based on card_type, default to IDC-
	...
	def before_save(self):
		...
		# Status Logging
		old_status = frappe.db.get_value("ID Card Generation", self.name, "card_status")
	...
	def log_print(self, layout="Single", bulk_print_id=None, total_cards=1):
		...
		if self.card_status != "Generated":
			frappe.throw("Cannot log print for non-generated card.")
		...
		# Determine Type
		# If it was 0, now it's 1 -> Original.
		# If it was 1, now it's 2 -> Reprint.
		p_type = "Original" if (current_count + 1) <= 1 else "Reprint"

		# Auto-generate bulk ID for single prints if missing
		...
		# We use get_person_doc logic or stored fields?
		# Stored fields are safer for audit history (snapshot).
		...
		log.print_action_type = "Bulk" if layout in ["Bulk"] else "Single"
		...
		if p_type == "Reprint":
			if layout == "Bulk":
				log.reprint_reason = "Bulk Generation Reprint"
			else:
				# If single, we might want to capture reason?
				# For now default to 'Manual Reprint' or passed arg?
				# Let's keep it simple.
				log.reprint_reason = "Manual Reprint"
